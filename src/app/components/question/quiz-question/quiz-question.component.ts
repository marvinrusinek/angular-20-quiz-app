import { AfterViewInit, ChangeDetectionStrategy, ChangeDetectorRef, Component, ComponentRef, EventEmitter, HostListener,
...
export class QuizQuestionComponent extends BaseQuestion
  ...
  async onVisibilityChange(): Promise<void> {
    ...
    if (document.visibilityState === 'hidden') {
      ...
      try {
        ...
        if (!this.quizId) {
          console.warn('[⚠️ Missing quizId in getQuestionState]');
          return;
        }
        const qState = this.quizStateService.getQuestionState(this.quizId, idx);

        const totalCorrectAnswers =
          Array.isArray(qState?.selectedOptions)
    ...
    try {
      if (document.visibilityState === 'visible') {
        ...
        // Purge only if user navigated to another question while hidden
        ...
        try {
          if (!this.quizId) {
            console.error('[QuizComponent] quizId is missing');
            return;
          }
    ...
    try {
      ...
      if (state?.isAnswered) {
        try {
          ...
          console.error(
            `❌ Failed to fetch explanation for Q${index}:`, explanationError
          );
      ...
      console.error(
        `Unexpected error during prepareQuestion for Q${index}:`, fatalError
      );
    }
  }
      ...
      if (
        ...
        (async () => {
          try {
            ...
            const formatted = ets.formatExplanation(canonicalQ, correctIdxs, raw).trim();
  ...
  private restoreFeedbackState(): void {
    try {
      ...
      console.error(
        '[restoreFeedbackState] Error restoring feedback state:', error
      );
    }
  }
  ...
  private stopTimerIfAllCorrectSelected(): void {
    ...
    const canonical = 
      (this.quizService.questions?.[idx]?.options ?? []).map(o => ({ ...o }));
    ...
    try {
      formatted =
        typeof ets.formatExplanation === 'function'
          ? ets.formatExplanation(q, q.options?.map((o, i) => (o.correct ? i + 1 : -1)).filter(n => n > 0), baseRaw)
          : baseRaw;
    } catch (e) {
  ...
  private async handleQuestionData(
    ...
    if (!this.quizId) {
      console.error('[getQuestionState] quizId is missing or invalid');
      return;
    }
    ...
    if (shouldShowExplanation) {
      try {
        ...
        ets.setResetComplete(true);
        ets.setExplanationText(explanationText);
        ets.setShouldDisplayExplanation(true);
        ets.lockExplanation();
      ...
      if (!ets.isExplanationLocked()) {
        ets.setExplanationText('');  // also clear stored explanation
        ets.setShouldDisplayExplanation(false);
        ets.setResetComplete(false);
      } else {
  ...
  async manageExplanationDisplay(): Promise<void> {
    try {
      ...
      if (!questionData) {
        throw new Error('[processExplanationText] No question data returned for index ' + this.currentQuestionIndex);
      }
      ...
      const ets = this.explanationTextService;
      ets.updateFormattedExplanation(explanationText);
      ets.setResetComplete(true);
      ets.setShouldDisplayExplanation(true);
      ets.lockExplanation();
      ...
      if (!this.quizId) {
        console.error('[manageExplanationDisplay] quizId is missing or invalid');
        return;
      }
  ...
  async prepareAndSetExplanationText(questionIndex: number): Promise<string> {
    ...
    try {
      ...
      if (!questionData) {
        throw new Error(`[getQuestionByIndex] No question found for index ${questionIndex}`);
      }
  ...
  private updateExplanationUI(questionIndex: number, explanationText: string): void {
    ...
    try {
      ...
      console.error(
        'Error in setting current question or updating explanation:', error
      );
    }
  }
  ...
  async onSubmit(): Promise<void> {
    ...
    const selectedControl = this.questionForm.get('selectedOption');
    if (!selectedControl) {
      console.warn('[getSelectedOption] selectedOption control not found');
      return;
    }
    const selectedOption = selectedControl.value;
  ...
  private validateForm(): boolean {
    ...
    const control = this.questionForm.get('selectedOption');
    if (!control) {
      console.warn('selectedOption control not found');
      return false;
    }
