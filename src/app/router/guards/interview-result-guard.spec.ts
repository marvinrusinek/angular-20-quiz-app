...
import { InterviewResultGuard } from './interview-result-guard';
...
describe('InterviewResultGuard', () => {
  let guard: InterviewResultGuard;
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(InterviewResultGuard);
  ...
  it('redirects to the builder when there is no completed result', () => {
    const result = guard.canActivate();
    expect(result).toBeInstanceOf(UrlTree);
    expect(router.serializeUrl(result as UrlTree)).toBe('/interview');
  });

  it('allows access after a result has been submitted', () => {
